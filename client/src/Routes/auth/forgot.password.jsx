...
const ForgotPasswordPage = ({ URL }) => {
  ...
  async function handleForgotPassword(e, email) {
          ...
          {success ? (
            <div className="login-right-info flex-column d-flex justify-content-center">
              ...
              <p className="fs-6 pb-4 text-muted">
                An email has been sent to your email address {email}. Follow the reset link to reset your passwod.
              </p>
            </div>
          ) : (
            // else
              ...
              >
                {forgotPasswordAuth.forgotPassworError && (
                  <p className="text-danger fw-bold">
                    {typeof forgotPasswordAuth.response === "object"
                      ? JSON.stringify(forgotPasswordAuth.response)
                      : forgotPasswordAuth.response}
                  </p>
                )}
