...
employeeRouter.put("/update-employee/:id", async (req, res, next) => {
  ...
  // do this if user isn't authorized to edit
  if (!authorization) {
  ...
  jwt.verify(token, "nkjsd;s5s68edsfdgdg8ds56r54KJhHGTFFYHTFYULHJDIUHSD", async (err, decoded) => {
    ...
    if (req.file) {
      ...
      try {
        ...
        const key = generateKey; // Leave it empty for now
        const file = req.file.buffer;

        // Upload file to S3
        await awsImageUpload(req, res, bucket, key, file);
        // File uploaded successfully, return URL or other relevant info

        const db = await connectDB();
        await db.collection("employees").updateOne(employeeInfoIntital, employeeInfoUpdated);
        const foundUser = await db.collection("employees").findOne({ email: req.body.email });
        console.log(`${foundUser}>> user in employees put`);
      ...
      try {
        const db = await connectDB();
        db.collection("employees").updateOne(employeeInfoIntital, employeeInfoUpdated);
